...
const QueueTraffic: React.FC = () => {
    ...
    useEffect(() => {
        const fetchQueues = async () => {
            ...
            setError(null);
            try {
                const res = await fetchWithAuth(`${API_URL}/queues/simple`);
                if (!res.ok) {
                    const errorData = await res.json();
                    throw new Error(errorData.message || 'Failed to fetch simple queues.');
                }
                const data: SimpleQueue[] = await res.json();
                setQueues(data.sort((a, b) => a.name.localeCompare(b.name)));
            } catch (err: any) {
    ...
    useEffect(() => {
        const pollTraffic = async () => {
            ...
            try {
                const res = await fetchWithAuth(`${API_URL}/queues/simple/traffic/${encodeURIComponent(selectedQueue)}`);
                if (!res.ok) {
                    const errorData = await res.json();
    ...
    const filteredQueues = useMemo(() => {
        ...
        if (!lowercasedQuery && !selectedQueue) return queues;
        if (!lowercasedQuery && selectedQueue) return queues;
        return queues.filter(q => q.name.toLowerCase().includes(lowercasedQuery));
    }, [queues, searchQuery, selectedQueue]);
    
    const handleSelectQueue = (queue: SimpleQueue) => {
        setSelectedQueue(queue.id);
        setSearchQuery(queue.name);
        setIsDropdownOpen(false);
    };
    ...
    return (
        <div className={rootClasses}>
            <h2 className={`text-2xl font-bold text-gray-800 dark:text-gray-100 ${isFullScreen ? 'hidden' : 'block'}`}>Simple Queue Traffic Monitor</h2>
            {error && !isFullScreen && <div className="p-4 bg-red-100 text-red-700 rounded-md">{error}</div>}
            
             <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-4 w-full sm:w-auto">
                    <label htmlFor="queue-select" className="font-semibold text-gray-700 dark:text-gray-300 flex-shrink-0">Select Queue:</label>
                    <div ref={dropdownRef} className="relative w-full sm:w-80">
                         <input
                            ...
                            placeholder={isLoading ? 'Loading...' : '-- Search for a queue --'}
                            ...
                            <div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-700 border dark:border-gray-600 rounded-md shadow-lg max-h-60 overflow-y-auto">
                                <ul>
                                    {filteredQueues.length > 0 ? (
                                        ...
                                        <li className="px-4 py-2 text-sm text-gray-500">No queues found.</li>
                                    )}
                                </ul>
                            </div>
                        )}
                ...
                <div className={chartWrapperClasses}>
                    {!selectedQueue ? (
                        <div className="flex items-center justify-center h-full text-gray-500 dark:text-gray-400">
                            <p>Please select a queue to begin monitoring.</p>
                        </div>
                    ) : (
                        <>
                            {isPolling && (

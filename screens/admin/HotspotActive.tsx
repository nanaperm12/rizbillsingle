...
import { HotspotActiveUser, formatBytes } from '../../types';
import { fetchWithAuth } from '~/components/api';

const API_URL = '/api';

const HotspotActive: React.FC = () => {
    ...
    const fetchData = useCallback(async (isInitialLoad = false) => {
        ...
        try {
            ...
            if (isOnline) {
                const res = await fetchWithAuth(`${API_URL}/hotspot/active`);
                if (!res.ok) {
            ...
            console.error("Failed to fetch active hotspot users:", err);
    ...
    const handleKickUser = async (userId: string) => {
        ...
        setError(null);
        try {
            const res = await fetchWithAuth(`${API_URL}/hotspot/active/${userId}/kick`, { method: 'POST' });
            const responseData = await res.json();
            if (!res.ok) {
                throw new Error(responseData.message || 'Failed to kick user');
            }
            setActiveUsers(responseData); // Update state directly from API response
        } catch (err: any) {
    ...
    const filteredUsers = useMemo(() => {
        ...
        return activeUsers.filter(u =>
            u.user.toLowerCase().includes(lowercasedQuery) ||
            u.address.toLowerCase().includes(lowercasedQuery)
        );
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Hotspot Active Users</h2>
                <button
                    ...
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3">Data In</th>
                                <th scope="col" className="px-6 py-3">Data Out</th>
                        ...
                        <tbody>
                            {paginatedUsers.length > 0 ? paginatedUsers.map(user => (
                                <tr key={user.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td className="px-6 py-4">{formatBytes(user.bytesIn)}</td>
                                    <td className="px-6 py-4">{formatBytes(user.bytesOut)}</td>
                                ...
                                <tr>
                                    <td colSpan={6} className="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                                       {isLoading ? 'Loading active users...' : !isRouterOnline ? 'Could not load active users. Router is offline.' : (searchQuery ? 'No users match your search.' : 'No active users found.')}
                                    </td>
                                </tr>
                            )}
                        </tbody>
